...
const HARVEST_MAPPINGS = [
  ...
  { key: 'PORTAL_ADMIN_PASSWORD', container: 'forensic-cert-portal', env: 'CERT_PORTAL_SECRET' },
  { key: 'PORTAL_ADMIN_USER', container: 'forensic-cert-portal', env: 'PORTAL_ADMIN_USER' },
];
...
function enrichFromProcessEnv(snapshot) {
  ...
  if (process.env.CERT_PORTAL_SECRET && !out.PORTAL_ADMIN_PASSWORD) {
    out.PORTAL_ADMIN_PASSWORD = process.env.CERT_PORTAL_SECRET;
  }
  if (process.env.SECRET_KEY && !out.PORTAL_ADMIN_PASSWORD) {
    out.PORTAL_ADMIN_PASSWORD = process.env.SECRET_KEY;
  }
  if (process.env.PORTAL_ADMIN_USER && !out.PORTAL_ADMIN_USER) {
    out.PORTAL_ADMIN_USER = process.env.PORTAL_ADMIN_USER;
  }
  return out;
}
