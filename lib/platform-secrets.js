 ...
 *
 * P-04 : CREDENTIAL_DEFAULTS et DEV_DEFAULTS ne sont utilisés qu'en mode dev
 * (isDevMode()). En production, une variable absente renvoie le fallback
 * fourni par l'appelant — jamais un mot de passe codé en dur.
 */
...
function getCredential(key, fallback = '—') {
  const v = process.env[key];
  if (v !== undefined && v !== '') return String(v);
  if (key === 'PORTAL_ADMIN_PASSWORD') {
    const portal = process.env.PORTAL_ADMIN_PASSWORD || process.env.CERT_PORTAL_SECRET || process.env.SECRET_KEY;
    if (portal) return String(portal);
  }
  if (key === 'PORTAL_ADMIN_USER') {
    const user = process.env.PORTAL_ADMIN_USER;
    if (user) return String(user);
  }
  const snap = loadCredentialSnapshot();
  if (snap[key] !== undefined && snap[key] !== '') return String(snap[key]);
  if (isDevMode() && CREDENTIAL_DEFAULTS[key] !== undefined) return CREDENTIAL_DEFAULTS[key];
  return fallback;
}
