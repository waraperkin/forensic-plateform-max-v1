...
function createThreatRoutes({ axios, logger, os, importToTimesketch }) {
  ...
  router.all('/*', async (req, res) => {
    ...
    const heavy = /^\/sekoia\/(fetch|events|search|volumetry|bulk|alerting|telemetry|intake)(\/|$)/.test(req.path);
    const timeout = heavy
