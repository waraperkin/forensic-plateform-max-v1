...
function createThreatRoutes({ axios, logger, os, importToTimesketch }) {
  ...
  router.all('/*', async (req, res) => {
    ...
    const heavy = /^\/sekoia\/(fetch|events|search|volumetry|bulk|alerting|telemetry|intake|graph|simulate|hosts|valuation|satisfiability|field-inventory|backtest|backtest-coverage)(\/|$)/.test(req.path);
    const timeout = heavy
