...
function createThreatRoutes({ axios, logger, os, importToTimesketch }) {
  ...
  const ALLOWED_PROXY_RE = /^\/(sekoia|s1)\/(assets|intakes|connectors|modules|playbooks|formats|rules|stats|apikeys|config|fetch|events|search|health|inventory|alerts|coverage|entities|local|anomalies|hosts|slo|forecast|effectiveness|mitre-coverage|watchlists|snapshots|digest)(\/|$)/;
