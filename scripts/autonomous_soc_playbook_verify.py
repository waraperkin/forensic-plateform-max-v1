#!/usr/bin/env python3
"""Vérifie Autonomous SOC Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, AUTONOMOUS_SOC_LAUNCHER  # noqa: E402
from osd_autonomous_soc_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[autonomous-soc-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if AUTONOMOUS_SOC_LAUNCHER not in pids:
                problems.append(f"bouton Autonomous SOC absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-asoc-s1-anomaly", "fp-asoc-s3-contain", "fp-asoc-s4-audit"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[autonomous-soc-verify] 0 problème(s) — OK")
    return 0
