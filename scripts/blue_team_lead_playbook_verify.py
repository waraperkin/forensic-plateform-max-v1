#!/usr/bin/env python3
"""Vérifie Blue Team Lead Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, BLUE_TEAM_LAUNCHER  # noqa: E402
from osd_blue_team_lead_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[blue-team-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if BLUE_TEAM_LAUNCHER not in pids:
                problems.append(f"bouton Blue Team absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-btl-s1-alerts", "fp-btl-s4-coverage"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[blue-team-verify] 0 problème(s) — OK")
    return 0
