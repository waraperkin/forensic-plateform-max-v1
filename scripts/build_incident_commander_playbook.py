#!/usr/bin/env python3
"""Génère fp-incident-commander-playbook.ndjson."""
...
NDJSON = OUT / "fp-incident-commander-playbook.ndjson"
sys.path.insert(0, str(ROOT / "scripts"))

from build_opensearch_dashboards import dashboard, finalize_dashboard  # noqa: E402
from osd_incident_commander_playbook_lib import DASH_ID, DASH_TITLE, append_searches, dashboard_panels  # noqa: E402
...
def build() -> list[dict]:
    ...
    for pid, pattern, tfield in [
        ...
        if not any(o.get("id") == pid and o.get("type") == "index-pattern" for o in objects):
            objects.append(
                obj("index-pattern", pid, {"title": pattern, "timeFieldName": tfield, "fields": "[]", "fieldFormatMap": "{}"})
            )
