#!/usr/bin/env python3
"""Vérifie CTI Fusion Global Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, CTI_FUSION_GLOBAL_LAUNCHER  # noqa: E402
from osd_cti_fusion_global_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[cti-global-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if CTI_FUSION_GLOBAL_LAUNCHER not in pids:
                problems.append(f"bouton CTI Global absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-ctfg-s1-opencti", "fp-ctfg-s4-ttp"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[cti-global-verify] 0 problème(s) — OK")
    return 0
