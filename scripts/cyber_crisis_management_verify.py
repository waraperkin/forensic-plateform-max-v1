#!/usr/bin/env python3
"""Vérifie Cyber Crisis Management Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, CYBER_CRISIS_LAUNCHER  # noqa: E402
from osd_cyber_crisis_management_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[cyber-crisis-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if CYBER_CRISIS_LAUNCHER not in pids:
                problems.append(f"bouton Cyber Crisis absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-ccm-s1-weak", "fp-ccm-s2-cell", "fp-ccm-s4-lessons"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[cyber-crisis-verify] 0 problème(s) — OK")
    return 0
