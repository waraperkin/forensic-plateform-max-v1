#!/usr/bin/env python3
"""Vérifie Global SOC Command Center Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, GLOBAL_SOC_LAUNCHER  # noqa: E402
from osd_global_soc_command_center_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[global-soc-verify] OK panels={len(panels)}")
        if not any(p.get("panelIndex") == SIDE_ID for p in panels):
            problems.append("side panel absent")
        if GLOBAL_SOC_LAUNCHER not in {p.get("panelIndex") for p in panels}:
            problems.append("barre playbooks absente du dashboard principal")
    ...
    for sid in ("fp-gscc-s2-global-attacks", "fp-gscc-s4-escalation", "fp-gscc-s3-mtta"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[global-soc-verify] 0 problème(s) — OK")
    return 0
