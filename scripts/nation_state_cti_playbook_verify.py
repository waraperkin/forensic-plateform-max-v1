#!/usr/bin/env python3
"""Vérifie Nation-State CTI Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, NATION_STATE_LAUNCHER  # noqa: E402
from osd_nation_state_cti_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[nation-state-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if NATION_STATE_LAUNCHER not in pids:
                problems.append(f"bouton Nation CTI absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-nsc-s1-campaigns", "fp-nsc-s3-ioc", "fp-nsc-s4-contain"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[nation-state-verify] 0 problème(s) — OK")
    return 0
