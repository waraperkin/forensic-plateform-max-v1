...
from osd_fp_playbooks_bars_lib import FP_DASHBOARDS_ALL, PURPLE_TEAM_LAUNCHER  # noqa: E402
from osd_purple_team_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[purple-team-verify] OK panels={len(panels)}")
    ...
    if not notebook_ok(s, NOTEBOOK_NAME):
        problems.append(f"notebook absent")
    if not app_ok(s, APP_NAME):
        problems.append(f"app absent")
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if PURPLE_TEAM_LAUNCHER not in pids:
                problems.append(f"bouton Purple absent: {dash_id}")
    for sid in ("fp-pt-s1-mitre-ttp", "fp-pt-s2-sigma", "fp-pt-s4-mitre-gap"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[purple-team-verify] 0 problème(s) — OK")
    return 0
