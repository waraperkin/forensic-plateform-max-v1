#!/usr/bin/env python3
"""Vérifie Red Team Lead Playbook."""
...
from osd_fp_playbooks_bars_lib import ALL_LAUNCHERS, FP_DASHBOARDS_ALL, RED_TEAM_LAUNCHER  # noqa: E402
from osd_red_team_lead_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[red-team-verify] OK panels={len(panels)}")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if r.status_code == 200:
            ...
            if RED_TEAM_LAUNCHER not in pids:
                problems.append(f"bouton Red Team absent: {dash_id}")
            miss = [lid for lid in ALL_LAUNCHERS if lid not in pids]
            if miss:
                problems.append(f"barre incomplète ({len(miss)} manquants) sur {dash_id}")
    for sid in ("fp-rtl-s1-campaigns", "fp-rtl-s4-mitre-gap"):
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        return 1
    print("[red-team-verify] 0 problème(s) — OK")
    return 0
