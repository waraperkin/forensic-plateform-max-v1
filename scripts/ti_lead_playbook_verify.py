...
from osd_fp_playbooks_bars_lib import FP_DASHBOARDS_ALL, TI_LEAD_LAUNCHER  # noqa: E402
from osd_ti_lead_playbook_lib import APP_NAME, DASH_ID, NOTEBOOK_NAME, SIDE_ID, search_specs  # noqa: E402
...
def main() -> int:
    ...
    if dr.status_code != 200:
        ...
        print(f"[ti-lead-verify] OK dashboard panels={len(panels)}")
    ...
    if not notebook_ok(s, NOTEBOOK_NAME):
        ...
        print("[ti-lead-verify] OK notebook")

    if not app_ok(s, APP_NAME):
        problems.append(f"app '{APP_NAME}' absente")
    else:
        print("[ti-lead-verify] OK application")
    ...
    for dash_id in FP_DASHBOARDS_ALL:
        ...
        if TI_LEAD_LAUNCHER not in pids:
            problems.append(f"bouton TI Lead absent: {dash_id}")

    for sid, label in [("fp-tl-s1-ingest", "ioc"), ("fp-tl-s2-malware", "cti"), ("fp-tl-s4-ioc-logs", "soc"), ("fp-tl-s5-ttp-gaps", "mitre")]:
    ...
    if problems:
        for p in problems:
            print(f"  - {p}")
        print(f"[ti-lead-verify] {len(problems)} problème(s)")
        return 1
    print("[ti-lead-verify] 0 problème(s) — OK")
    return 0
