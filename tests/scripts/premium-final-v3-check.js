...
const OUT = path.join(__dirname, '..', 'artifacts', 'ui-renovation', 'after-premium-final-v3');
...
function scanBody(text, label) {
  ...
  if (/\bmsg\.[a-z_]+\b/.test(text)) bad.push('cle i18n visible');
...
(async () => {
  ...
  // Token frais pour IT (genere APRES tous les autres, jamais consomme avant la capture mobile)
  await page.goto(`${BASE}/?tab=tokens`);
  await page.waitForTimeout(1000);
  await page.fill('#tk-case', 'CASE-V3-MOBILE');
  await page.click('button:has-text("GÉNÉRER LE TOKEN"), button:has-text("GENERATE TOKEN")');
  await page.waitForTimeout(1000);
  const mobileTokenUrl = (await page.locator('#gen-url').textContent()).trim();
  console.log('[Token mobile frais]', mobileTokenUrl);

  // Deuxieme token pour le test upload desktop (consommation separee)
  await page.fill('#tk-case', 'CASE-V3-DESKTOP-UPLOAD');
  await page.click('button:has-text("GÉNÉRER LE TOKEN"), button:has-text("GENERATE TOKEN")');
  await page.waitForTimeout(1000);
  const desktopTokenUrl = (await page.locator('#gen-url').textContent()).trim();
  console.log('[Token desktop upload]', desktopTokenUrl);
  if (!mobileTokenUrl.includes('localhost:8443') || !desktopTokenUrl.includes('localhost:8443')) anyBad = true;

  const itPage = await ctx.newPage();
  await itPage.goto(desktopTokenUrl);
  ...
  anyBad = scanBody(await itPage.locator('body').innerText(), 'IT mobile no-token').length > 0 || anyBad;

  // IT mobile avec token FRAIS non consomme
  await itPage.goto(`${mobileTokenUrl}#it-upload`);
  await itPage.waitForTimeout(1000);
  const lockedMobile = await itPage.evaluate(() => document.getElementById('main').classList.contains('it-locked'));
  console.log('[IT mobile avec token frais locked=]', lockedMobile);
  if (lockedMobile) anyBad = true;
  ...
  anyBad = scanBody(await itPage.locator('body').innerText(), 'IT mobile with-token').length > 0 || anyBad;

  await browser.close();
  console.log(anyBad ? 'RESULTAT: PROBLEMES DETECTES' : 'RESULTAT: OK — premium final v3 propre');
